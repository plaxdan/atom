  ...
  SYSTEM:
    INITIALIZE: ->
      ServerService.loadServer()
        # Status is 'local' or 'remote'
        .then (status) ->
          @dispatch Msg.SERVER.LOADED, status
          switch status
            when 'REMOTE', 'LOCAL' then
              session = SessionService.resume()
              if session
                @dispatch Msg.SESSION.AUTHENTICATED, session
              else
                @dispatch Msg.SESSION.DENIED, session

            when 'UNKNOWN'
              # THIS IS HANDLED IN THE UI BY THE ServerStore
