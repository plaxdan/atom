...
SERVICES:
  ...
  ConfigurationService:
    updateConfig: (onProgress) ->

  ConnectionService:
    constructor: (onDropped: => @dispatch Action.ACTION)
    connect: -> # Noop if already connected
    disconnect: -> # Noop if already disconnected
    startMonitor: (onResponse, onError) -> @intervalID = setInterval (-> ajax), 30 * 1000
