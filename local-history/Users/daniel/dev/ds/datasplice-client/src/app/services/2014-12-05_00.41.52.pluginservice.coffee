...
class PluginService
  ...
  _loadPlugin: (manifest) ->
    ...
    pluginMain = UrlHelper.prefix "/plugins/#{manifest.name}/#{manifest.main}"

    console.log "Services.pluginService: Loading file #{pluginMain}" if TRACE

    pluginName = (manifest.name).replace /-/g, '_'
    _handleJSONPCallback pluginName

    basket.require url: pluginMain, live: true, execute: true
