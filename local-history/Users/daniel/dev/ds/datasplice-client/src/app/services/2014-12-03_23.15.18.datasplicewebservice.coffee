...
# The DataSpliceWebService is a stateless utility service which interacts with
# the REST API of a DataSplice server.
#
...
class DataSpliceWebService
  ...
  _request: (endPoint, method, options = {}) ->
    { silentErrorsMatching } = options
    delete options.silentErrorsMatching
    url = "#{@_serverAddress}#{endPoint}"
    # httpinvoke's promise has no 'catch' method so we wrap it.
    new Promise (resolve, reject) =>
      success = (response) ->
        if options.forceParse and _.isString response.body.payload
          response.body.payload = JSON.parse response.body.payload
        resolve response.body
      fail = (error) ->
        unless silentErrorsMatching?.test error.message
          console.error "Request to #{endPoint} failed with:", error
        reject error
      @_client url, method, options
        .then success, fail

  _get: (endPoint, options = {}) ->
    console.log "Services.webService GET #{endPoint}" if TRACE
    @_request endPoint, 'GET', options

  _post: (endPoint, payload, options = {}) ->
    if TRACE
      console.log "Services.webService POST #{endPoint} with payload: \
        #{payload}"
    if payload
  ...
  _initialized: ->
    logMsg = "[DataSpliceWebService] initialized with: \
      \n\tServer address:\t[#{@_serverAddress or 'NONE'}]"
    console.info logMsg
    ...
    onFinished = (error, body, statusCode, headers) ->
      ...
      unless error
        ...
        if statusCode is 200
          payload =
            if body and /json|javascript/.test headers['content-type']
              try
                JSON.parse body
              catch
  ...
  # GET /ds/push
  getPush: (options = {}) ->
    # don't clutter up the console with log messages from push requests
    options.silentErrorsMatching = /timeout/
  ...
  # POST /ds/session/revoke/<sessionName>
  revokeSession: (sessionName) ->
    @_post "#{@_endPoints.revoke}/#{sessionName}"
  ...
  getPlugins: ->
    # bing - the DataSplice server is responding with text/plain at the
    # moment for this request, so the contents aren't parsed
    # @_get @_endPoints.pluginmanifest, forceParse: true

    new Promise (resolve, reject) =>
      loadOpts =
        url: @_endPoints.pluginmanifest
        live: true
        execute: false

      basket.require loadOpts
        .then ->
          basket.get @_endPoints.pluginmanifest
        .then (response) ->
          console.debug "Basket response: ", response
