...
Header = React.createClass
  ...
  mixins: [
    FluxMixin
    StoreWatchMixin Stores.ScannerIntegration, Stores.Session,
      Stores.TopLevelActions
  ]

  getStateFromFlux: ->
    scanner: (@getFlux().store Stores.ScannerIntegration).getState()
    sessionData: (@getFlux().store Stores.Session).getState()
  ...
  render: ->
    ...
    div null,
      AppMenu
        serverInstance: @state.sessionData.instanceName
      ...
      if @state.messageCenterSection
        MessageCenter
          sessionMode: @state.sessionData.mode
          userIdentifier: @state.sessionData.userIdentifier
