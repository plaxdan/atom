...
```
datasplice-plugin-leaflet/dist/Server/Resources/WebClient/plugins/datasplice-plugin-leaflet
```
...
Distribute the leaflet plugin:
    ...
    gulp clean && gulp dist --distpath=../datasplice-client/build/ --nouglify --sourcemap --trace

The `--nouglify`, `--trace` and `--sourcemap` flags are available for debugging. Verify the plugin has been recognized in the console:
