module.exports = (dsApp) ->
  ...
  {ExpressionEvaluator, FilterItem} = dsApp.utils
  ...
  FeatureLayer = L.GeoJSON.extend
    ...
    onEachFeature: (feature, layer) ->
      ...
      # also select features that are already marked for selection
      else if @_selectedOnMap[feature.id] and @options.selection?.style
        @_addFeatureToSelection feature.id, @options.selection.style
      ...
      query.dataSet.on 'reset', =>
        features = for record in query.dataSet.models
          if @_featuresOnMap[record.id]
            ...
            geoJSON = @_recordToGeoJson record
            geoJSON
    ...
    _queryGeoJsonFeatures: (bounds) ->
      filter = [
        ...
        "'Y Min' >= #{bounds.getSouth()}"
        "'Y Max' <= #{bounds.getNorth()}"
      ]
      query = @_dataController.createQuery @name,
        # TODO :This forces local queries because remotes aren't yet supported.
        # localData: true
        filter: filter.join ' and '

      query.dataSet.on 'reset', =>
        console.warn query.dataSet
    ...
    _addFeatureToSelection: (id, style) ->
      if @_featuresOnMap[id]
        ...
        ($.when StyleHelper.processStyleReferences style, context, ExpressionEvaluator)
          .done (style) ->
            layer.setStyle style

      @_selectedOnMap[id] = id
