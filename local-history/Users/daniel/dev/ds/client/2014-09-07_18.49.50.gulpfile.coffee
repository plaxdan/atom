...
# Include assetss may be strings or objects. Bundles are always strings. The dest of
# an include has the following precedence:
#   1. include.assets[idx].dest (if asset[idx] is an object)
#   2. include.dest (if asset[idx] is a string or an object with no dest property)
