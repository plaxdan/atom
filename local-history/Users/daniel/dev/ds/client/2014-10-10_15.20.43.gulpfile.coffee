...
vendors = [
  ...
  {
    name: 'classlist.js'
    base: './bower_components/classlist.js'
    bundle: [
      'classList.js'
    ]
  }
...
<<<<<<< HEAD
    unless $.util.env.nosourcemap or $.util.env.nonothing
      webpackConfig.devtool = 'sourcemap'
=======
    webpackConfig.devtool = 'source-map' unless $.util.env.nosourcemap
>>>>>>> WIP
...
gulp.task 'build:html', ->
  indexParams = ->
    ...
    params.manifest = if $.util.env.nomanifest or $.util.env.nonothing
      ''
    else
      'manifest="offline.manifest"'
    params
  nolr = $.util.env.nolr or $.util.env.nonothing
  gulp.src "#{srcPath}/html/index.html"
    # embeds the live reload script
    .pipe if $.util.env.production or nolr
        $.util.noop()
      else
        $.embedlr port: lrPort
...
gulp.task 'build:tests', [ 'build:src', 'build:vendor' ], ->
  ...
  gulp.src "#{srcPath}/html/test.html"
    # embeds the live reload script
    .pipe if nolr then $.util.noop() else $.embedlr port: lrPort
    .pipe gulp.dest webBuildPath
...
gulp.task 'build:offlinemanifest', (cb) ->
  if $.util.env.nomanifest or $.util.env.nonothing
    cb()
  else
  ...
  for vendor in vendors
    ...
    if vendor.include?
      ...
      for asset in vendor.include.assets
        src = if _.isString asset
          "#{vendor.base}/#{asset}"
        else if _.isObject asset
          "#{vendor.base}/#{asset.src}"
        else
          throw new Error 'asset must be a string or object'

        # asset.dest should always be relative to its parent
        dest = if vendor.include.dest
          # parent is include.dest
          "#{vendor.include.dest}/#{asset.dest or ''}"
        else
          # parent is #{vendorPath}/#{vendor.name}
          "#{vendorPath}/#{vendor.name}/#{asset.dest or ''}"

        gulp.src src
          .pipe gulp.dest dest

        $.util.log "    #{src} #{magenta '->'} \
          #{green path.relative '.', dest}"
