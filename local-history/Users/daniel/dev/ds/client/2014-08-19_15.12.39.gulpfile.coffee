...
{ red, cyan, blue, green, magenta, black } = $.util.colors
...
gulp.task 'build:vendor', ->
  ...
  for vendor in vendorAssets
    ...
    assetDirName = undefined
    assetBaseName = undefined
    ...
    for asset in vendor.assets
      ...
      if vendor.global or asset.global
        ...
        assetRelativePath = path.relative '.', asset.src
        nextAssetDirName = path.dirname assetRelativePath
        assetBaseName = path.basename assetRelativePath

        $.util.log "#{nextAssetDirName}/" unless nextAssetDirName is assetDirName
        $.util.log "\t#{black assetBaseName} -> " +
        ...
        $.util.log "\t#{path.relative '.', asset.src} -> " + magenta "#{path.relative '.', asset.dest}"
