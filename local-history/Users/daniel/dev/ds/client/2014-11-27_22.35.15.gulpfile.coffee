...
gulp.task 'build', [ 'build:skel', 'build:app', 'build:webapp' ]
...
gulp.task 'run:mocha', ->
  reporter = if $.util.env.ci then 'mocha-jenkins-reporter' else 'dot'
  $.run "./node_modules/.bin/mocha-phantomjs -R #{reporter} #{paths.dist}/test.html"
