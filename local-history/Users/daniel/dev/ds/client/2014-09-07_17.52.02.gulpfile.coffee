...
gulp.task 'build:vendor2', ->
  ...
  bundledVendorsRelativeFilePath =
    "#{path.relative '.', vendorPath}/#{vendorBundleFile}"
  ...
  for vendor in vendors
    ...
    if vendor.include?
      ...
      for asset in vendor.include.assets
        if _.isString asset
          unless asset.dest?
            throw "Must specify include.dest when assets are \
              defined as strings"
          asset =
            src: "#{vendor.base}/#{asset}"
            dest: asset.dest or include.dest
        else if _.isObject asset
          asset.src = "#{vendor.base}/#{asset.src}"

        asset.dest or= "#{vendorPath}/#{vendor.name}"
