  ...
  for vendor in vendorAssets
    ...
    for asset in vendor.assets
      ...
      if _.isString asset
        ...
        # Vendor or asset must have a dest or be global
        unless vendor.dest or vendor.global
          unless asset.dest or asset.global
            $.util.log red "error - if #{vendor.name} has no dest or isnt \
            global then asset must have a dest or be global"
            return false
        asset.src = "#{vendor.base}/#{asset.src}"
        asset.dest = if asset.dest? then asset.dest else ''
