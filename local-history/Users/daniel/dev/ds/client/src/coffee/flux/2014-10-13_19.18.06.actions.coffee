...
module.exports = (factory) ->
  ...
  packageSession = (session) ->
    console.warn 'PACKAGING SESSION'
    new Promise (resolve, reject) ->
      sessionService.getOfflineBehavior session
        .then (offlineBehavior) ->
          console.warn 'GOT OFFLINE BEHAVIOR', offlineBehavior
          resolve _.assign {offlineBehavior}, session.attributes
        ...
        .then (server) =>
          ...
          sessionService.resume()
            .then (session) =>
              packageSession session
                .then (sessionPackage) ->
                  console.warn "Session package", sessionPackage
