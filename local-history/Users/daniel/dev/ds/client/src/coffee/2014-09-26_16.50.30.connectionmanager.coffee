...
class ConnectionManager
  ...
  constructor: (@factory, @dataSpliceWebServcice) ->
    {@pubSub} = @factory
  ...
  _generateOfflineDigest: ->
    ...
    @dataSpliceWebServcice.getConfigurationDigests()
    .then (result) =>
    ...
    # $.ajax
    #   url: UrlHelper.prefix 'ds/session/digest'
    #   dataType: 'json'
    #   complete: (response) =>
    #     # compare the server digest to the local data to see what we need to
    #     # send as an extended digest
    #     localDigest = {}
    #     serverDigest = response.responseJSON
    #     keys = _.keys serverDigest
    #     statusCount = 0
    #     async.each keys,
    #       (view, done) =>
