...
SessionService = (factory) ->
  ...
  login: (username, password, domain) ->
    new Promise (resolve, reject) ->
      ...
      session.save null,
        success: ->
          SessionService._sessionLoaded session
          resolve session
  ...
  resume: ->
    new Promise (resolve, reject) ->
      ...
      session.fetch
        bypassCache: true
        success: ->
          resolve SessionService._sessionLoaded session
        error: (model, error) ->
  ...
  _sessionLoaded: (session) ->
    ...
    eventFactory.execute EventRegistry.LoggedIn, context
  ...
  _initializeLocalization: ->
    ...
    eventContext = factory.eventFactory.context()
