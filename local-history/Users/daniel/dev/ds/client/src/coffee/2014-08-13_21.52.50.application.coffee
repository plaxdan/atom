...
CoreApplicationView = require './views/core'
...
class DataSpliceApplication extends Backbone.Router
  ...
  initialize: (options) ->
    super options
    @pubSub.on 'replaceView', =>
      @_currentFragment = Backbone.history.fragment
      @_routed = true
    @pubSub.on 'sessionLoaded', @sessionLoaded
    @pubSub.on 'serverError', @loadError
    @pubSub.on 'sessionError', @loadError
  ...
  doSessionSelection: ->
    @pubSub.trigger 'replaceView', @viewFactory.create SessionSelectionView

  doHome: ->
    $.when(@validate()).done =>
      @pubSub.trigger 'replaceView', @viewFactory.create HomeView

  doSettings: (action) ->
    $.when(@validate()).done =>
      try
        path = if action then (decodeURIComponent action).split '/' else []
        @pubSub.trigger 'replaceView', @viewFactory.create ApplicationSettingsView,
          { path }
      catch error
  ...
  doDisplayView: (path, query) ->
    $.when(@validate()).done =>
      ...
      @pubSub.trigger 'replaceView', @viewFactory.create CompositeDataView,
        { state }
  ...
  run: ->
    ...
    # hook up the modal manager component
    modal = ($ '<div class="modal-container">').appendTo $ 'body'
    props =
      factory: @viewFactory
      pubSub: @pubSub
    React.renderComponent (ModalManager props), modal[0]

    # >>>>
    # REPLACE THE CoreApplicationView
    # <<<<
    mountPoint = (document.getElementsByTagName 'body')[0]
    ui = UI factory: @viewFactory
    React.renderComponent ui, mountPoint

    # display something until the appController initializes things
    @coreView = @viewFactory.create CoreApplicationView
    ($ 'body').append @coreView.render().el
