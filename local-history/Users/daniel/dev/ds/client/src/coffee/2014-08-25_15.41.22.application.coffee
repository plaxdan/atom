...
class DataSpliceApplication extends Backbone.Router
  ...
  initialize: (options) ->
    super options
    @pubSub.on 'currentView', _.bind @doCurrentView, @
    ...
    @pubSub.on 'navigate', _.bind @routeNavigation, @
  ...
  doCurrentView: (callback) ->
    contentView = @reactUI?.props.component?.props.contentView
    # This needs to always fire the callback regardless of whether or not the
    #   view is defined (the caller should receive null)
  ...
  doHome: ->
    $.when(@validate()).done =>
      ...
      # build the actions for each startcenter category
