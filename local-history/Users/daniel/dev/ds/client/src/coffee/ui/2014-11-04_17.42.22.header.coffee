...
Header = React.createClass
  ...
  mixins: [
    FluxMixin
    StoreWatchMixin FluxStores.Server, FluxStores.Session, FluxStores.TopLevelActions,
      FluxStores.ScannerIntegration
  ]

  getStateFromFlux: ->
    server: (@getFlux().store FluxStores.Server).getState()
    session: (@getFlux().store FluxStores.Session).getState()
    topLevelActions: (@getFlux().store FluxStores.TopLevelActions).getState()
    scanner: (@getFlux().store FluxStores.ScannerIntegration).getState()
  ...
  componentWillReceiveProps: (nextProps) ->
    ...
    if @state.scanner.uniwedgeStatus is 'unknown'
      @getFlux().intents.hardware.updateScannerConfig()
    ...
    div null,
      ...
      if @state.messageCenterSection
        MessageCenter
          ...
          displayViewAction: _.compose @_hideMessageCenter,
            @props.displayViewAction

  _triggerScan: ->
    @getFlux().intents.hardware.triggerScan activeElement: @_activeElement
  ...
  _logOut: ->
    @_hideMessageCenter()
    @getFlux().intents.session.logout()
  ...
  _displayHome: ->
    @_hideMessageCenter()
    @getFlux().intents.navigate.home()

  _displaySettings: ->
    @_hideMessageCenter()
    @getFlux().intents.navigate.settings()

  _displayModifications: ->
    @_hideMessageCenter()
    @getFlux().intents.navigate.settings 'modifications'
  ...
  _resetModification: (cid) ->
    ...
    dataController.findRecord { modification }, (record) =>
      state = dataController.createRecordDisplayState record
      controller = new DisplayStateController state, @props.factory, component:@
