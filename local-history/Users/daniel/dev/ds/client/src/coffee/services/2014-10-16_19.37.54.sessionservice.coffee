...
class SessionService
  ...
  constructor: (@_factory, @_configService) ->
    ...
    new Promise (resolve, reject) =>
      ...
      session.fetch
        ...
        success: (model, response, options) =>
          ...
          if weHaveNoConfig
            @_configService.loadRemote config
              .then =>
                @_notifyDataSplice session
                  .then ->
                    resolve session
        error: (model, error) =>
          ...
          if (error.status isnt 400) and session.loadLocal()
            # Load config if necessary
            Promise.resolve @_loadLocalConfig session
              .then =>
                @_notifyDataSplice session
                  .then ->
                    resolve session
  ...
  _loadLocalConfig: (session) ->
    config = session.get 'views'
    weHaveConfig = config.length > 0
    return true if weHaveConfig
    @_configService.loadLocal config
