...
ActionDataPane = React.createClass
  ...
  shouldComponentUpdate: (nextProps, nextState) ->
    ...
    #   being processed by the ExpressionEvaluator.
    activeState = nextProps.controller.activeState()
    if activeState
      actionProps = (state.get 'action')?.properties?.Attributes
      if actionProps and not nextState.actionProps
        # wait until the actionprops have been evaluated and set on the state
        false
      else
        # action props have already been evaluated or they would not be on state
        true
    else
      # continue, no need to wait for expression evaluator
      true
