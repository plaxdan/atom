class ConfigService
  ...
  # Usage:
  #   configService.getActions session, DSActions.HomeScreenActions, groupBy: 'category'
  getActions: (session, dsActionType, options) ->
    new Promise (resolve, reject) ->
      ...
      processConfig = (accumulator, candidate) ->
        ...
        if (candidate.get 'navigationActions')[dsActionType]?
          if _.isArray accumulator
            accumulator.push candidate
          else
            # Example: candidate['category'] is 'Work Orders'
            group = candidate[groupBy]
            # ignore if the action does not have a non-empty group field
            if groupBy? and group
              accumulator[group] or= []
              accumulator[group].push candidate
