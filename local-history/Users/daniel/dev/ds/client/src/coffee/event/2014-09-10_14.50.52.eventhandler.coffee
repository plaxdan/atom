...
class EventHandler
  ...
  DeleteRecordsAction: (action) ->
    ...
    new Promise (resolve, reject) =>
      ...
      deleteRecord = (recordContext, done) ->
        mh.markDeleted recordContext.record, context: recordContext
          .then done
  ...
  IterateCommandAction: (action) ->
    ...
    new Promise (resolve, reject) =>
      ...
      iterateRecord = (recordContext, done) ->
        ExpressionEvaluator.evaluate command, context: recordContext
          .then done
          # bing - see comment below in ModifyDataAction
          .catch (error) -> reject error
  ...
  ModifyAttributesAction: (action) ->
    ...
    ExpressionEvaluator.evaluateAttributes attributes, {@context}
      .done =>
  ...
  ModifyDataAction: (action) ->
    ...
    new Promise (resolve, reject) ->
      ...
      # process and iterate the query
      @_iterateActionQuery actionOptions, modifyRecord, @
  ...
  ModifyFilterAction: (action) ->
    ...
    unless @context.dataSet
      reject 'Cannot modify filter in this context'
    else
      query = @context.dataSet.query

      filter = new FilterItem actionOptions.filterOption
      filter.replaceAttributeReferences @context
        .then ->
    ...
    new Promise (resolve, reject) =>
      async.map (_.pluck actionOptions.parameterBindings, 'parameterValue'),
        (param, done) =>
          ...
          if _.isObject param
            param = _.clone param
            ExpressionEvaluator.evaluateAttributes param, { @context }
              .then (evaluated) ->
            ...
            if ExpressionEvaluator.isExpression param
              ExpressionEvaluator.evaluate param, { @context }
                .then (evaluated) ->
        ...
        , (err, paramInfo) =>
          ...
          if actionOptions.disposition isnt 'Server'
            reject 'Client-side plug-in function actions are not implemented at the moment...'
          else
            data =
    ...
    # enabled, this happens during offline sync in some cases
      ...
      if @options?.postponeErrors
        if promptOptions.acceptButton and promptOptions.cancelButton
          reject 'Cannot display interactive prompt'
        else if promptOptions.cancelButton
          reject promptOptions.promptMessage
        else
          resolve()
      else
  ...
  _iterateActionQuery: (action, recordCallback, promise) ->
    ...
    promise.then hideWait, hideWait
