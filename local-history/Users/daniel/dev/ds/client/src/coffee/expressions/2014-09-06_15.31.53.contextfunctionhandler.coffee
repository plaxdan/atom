...
ExpressionTestPrompt = React.createClass
  ...
  componentDidMount: ->
    @props.factory.pubSub.trigger 'currentEventContext', (context) =>
      @setState availableFunctions: context.availableFunctions()

  mixins: [ React.addons.LinkedStateMixin ]
  render: ->

    if @state.availableFunctions?
      allFuncs = _.flatten @state.availableFunctions
      funcList = allFuncs.join ', '
...
class ContextFunctionHandler extends ExpressionFunctionHandler
  ...
  findRecord: (values, options) ->
    ...
    query.dataSet.on 'reset', ->
      ...
      if matches.length
        ...
        # matches collection
        first = _.indexOf options.context.dataSet.models, (test, index) ->
  ...
  _doMove: (delta, relative, options) ->
    iface = @_assertDisplayInterface options
    $.when(iface.selectRecord delta, relative: relative)
      .done( ->
  ...
  homeScreen: (values, options) ->
    @factory.pubSub.trigger 'displayHome'
    true
