...
class SessionService
    ...
    new Promise (resolve, reject) =>
      ...
      ef.execute EventRegistry.LogOut, ef.context(), (context) =>
        Promise.resolve @_legacyFactory.modificationHandler.commitChanges()
          .then =>
            afterDestroySession = =>
              @_clearSession @_legacyFactory.session
              @_legacyFactory.connectionManager.disconnect silent: true
              resolve()
  ...
  _sessionError: (session) ->
    ...
    console.log 'Services.session._sessionError: \'Failed to load session\''
