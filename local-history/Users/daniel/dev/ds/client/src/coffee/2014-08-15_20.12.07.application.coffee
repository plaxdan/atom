...
class DataSpliceApplication extends Backbone.Router
  ...
  initialize: (options) ->
    super options
    @pubSub.on 'replaceView', @doReplaceView
  ...
  doSessionSelection: ->
    # TODO: This is handled in UI now
  ...
  doHome: ->
    $.when(@validate()).done =>
      @reactUI.setProps contentView: @viewFactory.create HomeView

      # TODO: this class subscribes to replaceView and sets @_currentFragment
      #   when this event occurs. Commenting the following out means that the
      #   @_currentFragment value will not be set.
      #
