...
UI = React.createClass
  ...
  mixins: [
    FluxMixin
    StoreWatchMixin FluxStores.Route,
                    FluxStores.Server
                    FluxStores.Session
                    FluxStores.Settings
  ]
  ...
  getStateFromFlux: ->
    ...
    route = (@getFlux().store FluxStores.Route).getState()
      ...
      switch @state.server.status
        ...
        when FluxMessages.SERVER_LOADED
          switch @state.session.status
            ...
            #     # TODO: Do we want to show a splash screen here?
            #     div {}
            when FluxMessages.SESSION_CREATING, FluxMessages.SESSION_RESUMING
  ...
  _getDocumentTitle: ->
    displayStateController = @state.routeProps?.displayStateController
  ...
  _displayViewAction: (action, controller) ->
    @getFlux().intents.navigate.view action, controller
