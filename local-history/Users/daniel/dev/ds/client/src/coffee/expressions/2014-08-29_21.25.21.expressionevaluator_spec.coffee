...
describe 'Expression Evaluator', ->
  ...
  checkStatements = (statements, context, done) ->
    ...
    , # iterate
      (item, callback) ->
        ...
        When(ExpressionEvaluator.evaluate statement, {context})
          .done (value) ->
  ...
  describe 'Attribute handling', ->
    ...
    it 'lookup attribute values', (done) ->
      ...
      When(ExpressionEvaluator.evaluate '${foo}', {context})
        .done (value) ->
          expect(value).to.equal 'bar'
          done()

    it 'return an error if an invalid attribute is referenced', (done) ->
      When(ExpressionEvaluator.evaluate '${foo}')
        .done ->
  ...
  describe 'Nested Attributes', ->
    ...
    it 'pass numerical attribute references to nested definitions', (done) ->
      ...
      When(ExpressionEvaluator.evaluate '${do_upper("something")}', { context })
        .done (value) ->
    ...
    it 'handle attribute parameters that are attribute references', (done) ->
      ...
      When(ExpressionEvaluator.evaluate '${do_upper(${bar})}', { context })
        .done (value) ->
          expect(value).to.equal 'SOMETHING'
          done()
