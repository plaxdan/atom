...
class DataSpliceWebService
  ...
  _get: (endPoint, options) ->
    new Promise (resolve, reject) =>
      ...
      handleResponse = (response) -> resolve _packageResponse response
      handleError = (err) -> reject _packageResponse err
  ...
  _post: (endPoint, payload, options) ->
    ...
    .then (response) -> resolve {payload: response.body, response}, (response) -> reject response
  ...
  _configureHTTPClient: ->
    httpinvoke.hook 'finished', (error, body, statusCode, headers) ->
      packageArgs = (error, body, statusCode, headers) ->
        payload = try
          JSON.parse body
        catch
          body
        ...
        alternateArgs = [
          error
          # body is now {payload: [JSONObject], response: [other args]}
          responseObject
          statusCode
          headers
        ]
      if statusCode is 200
        packageArgs arguments...
      else [
        new Error "Server or client error - HTTP status #{statusCode}"
        output
        statusCode
        headers
      ]
