...
describe 'DEBUG flag', ->
  it 'should set window.React if DEBUG is truthy', ->
    if DEBUG
      (expect window.React).to.not.be.null

  it 'should NOT set window.React if DEBUG is falsy', ->
    unless DEBUG
      (expect window.React).to.be.null
...
describe 'Promise Handling', ->
  ...
  it 'should support Promise.cast for values that are not promises', (done) ->
    (Promise.cast 'static')
      .done (result) ->
        expect(result).to.equal 'static'
        done()

  it 'should support Promise.cast for values that are promises', (done) ->
    p = new Promise (resolve) ->
      resolve 'promise'
