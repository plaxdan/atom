...
class DisplayStateController
  ...
  stateEvents:
    'change:selectedField': '_onFieldSelect'
    ...
    if @_modalController
      ...
      if record
        ...
        $.when(mh.verifyRecord record, context: options?.context)
          .done( => @_triggerStateEvent state, 'validate:success', { record } )
          .fail =>
  ...
  _onFieldSelect: (state, fieldName, options) ->
    console.debug 'DisplayStateController._onFieldSelect', arguments if TRACE
    previous = state.previous 'selectedField'
    if previous? and previous isnt state.get 'selectedField'
      console.log 'blah'
