...
class DisplayStateController
  ...
  validate: (options) ->
    ...
    if @_modalController
      ...
      if record
        ...
        Promise.resolve mh.verifyRecord record
          .then => @_triggerStateEvent state, 'validate:success', { record }
          .catch =>
  ...
  fetchQuery: (options) ->
    Promise.resolve @validate()
      ...
      .catch (err) ->
        console.error "Error resolving validation promise", err
        throw err
  ...
  createRecord: (options) ->
    ...
    ($.when @validate())
      .done =>
        ...
        Promise.resolve @factory.modificationHandler.createRecord ds, createOpts
          .then (record) =>
            state.selectRecord record
            unless options?.nonInteractive
              Promise.resolve @interactiveEdit { state }
                .then ->
  ...
  _handlePerformActionResults: (results, state) ->
    ...
    $.when(@_updateFromContext()).done =>
      if state.canQuery and action?.displayMode in [ 'filter', 'search' ]
        @displayInterface.toggleSearch true
  ...
  _invokePopupResultsEvent: (action, results, context) ->
    ...
    if results.role is 'accept' and action.confirmEvent
      ...
      Promise.resolve ExpressionEvaluator.evaluateAttributes attrs, context: temp
        .then =>
