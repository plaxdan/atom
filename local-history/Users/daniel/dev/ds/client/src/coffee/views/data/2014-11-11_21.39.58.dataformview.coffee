...
class DataFormView extends BaseFormView
  ...
  focusChanged: ->
    field = @focusedField()
    selectedField = field?.input.binding.key
    previouslySelected = @state.get 'selectedField'
    console.debug "DataFormView.focusChanged from #{previouslySelected} to #{selectedField}" if TRACE
    unless selectedField is previouslySelected
      @state.set 'selectedField', selectedField
  ...
  selectedFieldChanged: ->
    ...
    console.debug "DataFormView.selectedFieldChanged #{fieldName}" if TRACE
  ...
  selectField: (fieldToSelect) ->
    console.debug "DataFormView.selectField" if TRACE
    field = @getEditor fieldToSelect
    field?.focus()
