...
ActionDataPane = React.createClass
  ...
  render: ->
    ...
    if @state.actionPlugin
      ...
      plugin.createDisplayComponent @props.displayMode, paneProps

    else if @props.displayMode in [ 'record', 'form ']
      RecordForm paneProps
    ...
    # handle modes still serviced by Backbone views
      ...
      if viewClass
        paneProps.viewClass = viewClass
        BackboneWrapper paneProps
      else
        div {},
  ...
  resize: (options) ->
    ...
    maxHeight = options?.maxHeight or @props.maxHeight
