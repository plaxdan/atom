...
class EventHandler
  ...
  constructor: (@identifier, @context, actions, @defaultHandler, @dataSpliceWebService, options) ->
  ...
  @performCreateRecordAction: (action, context, factory) ->
    ...
    $.when(ExpressionEvaluator.evaluateAttributes values, { context })
      .done ->
  ...
  ExecuteCommandAction: (action) ->
    ...
    $.when(ExpressionEvaluator.evaluate statement, {@context}).always =>
    ...
    async.map (_.pluck actionOptions.parameterBindings, 'parameterValue'),
      ...
      , (err, paramInfo) =>
        ...
        if actionOptions.disposition isnt 'Server'
          ...
          @dataSpliceWebService.performPluginAction data
            .then (result) =>
              { payload } = result
              updates = {}
            ...
            .catch (error) ->
              ...
              userError = (new UserError message).toString()
