...
class CoreApplicationView extends BaseView
  ...
  render: ->
    ...
    # start with the menu hidden - it will get shown once the session is loaded
    @messageCenter = @registerChildView @options.factory.create MessageCenterView
    @sessionMenu = @registerChildView @options.factory.create SessionMenuView,
      { @messageCenter }
    ok.append @sessionMenu.render()
    @sessionMenu.$el.hide()

    ok.append @sessionMenu.render()
    ok.append @messageCenter.render()
