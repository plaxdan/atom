...
class DataSpliceApplication extends Backbone.Router
  ...
  initialize: (options) ->
    ...
    @pubSub.on 'displayHome', =>
      console.debug 'Backbone displayHome fired'
      @navigate 'ui/home', trigger: true
  ...
  doHome: ->
    console.debug 'Application: doHome'
    $.when(@validate()).done =>
      @reactUI.setProps
  ...
  sessionLoaded: =>
    ...
    _.defer =>
      ...
      console.debug "Application: sessionLoaded with fragment: #{fragment}"
