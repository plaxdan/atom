...
class ConnectionManager
  ...
  lockSession: (options) ->
    ...
    @intents.session.lock()

    # TODO: move this into the session intent
    @pubSub.trigger 'displayModal', ConfirmPasswordPrompt
      instanceName: @legacyFactory.server.get 'instanceName'
      userName: @legacyFactory.session.userName()
      confirmPassword: (password) =>
  ...
  monitorConnection: ->
    return if @_currentPush or @legacyFactory.session.isEmpty() or @isDisconnected()
    ...
    tryReconnect = (error) =>
      unless (error.message in ['OK', 'timeout', 'abort']) or @isDisconnected()
        if error.payload
  ...
  _changeState: (state, options) ->
    ...
    if TRACE
      console.log "ConnectionManager._changeState #{@state} -> #{state}"
    ...
    # we can reconnect
    if @isDropped() and not options?.silent
      @_tryReconnect 5000
