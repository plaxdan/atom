...
UI = React.createClass
  ...
  render: ->
    console.info "Rendering UI with state: #{JSON.stringify @state}"
    app = [
      Header factory: @props.factory
      Content factory: @props.factory, view: @props.contentView
    ]

    div id: 'ui', className: 'page-container',
      ModalManager factory: @props.factory, pubSub: @props.factory.pubSub
      Header factory: @props.factory if @state.serverLoaded and @state.sessionLoaded
      Content factory: @props.factory if @state.serverLoaded and @state.sessionLoaded
        # app
      if @state.serverLoaded and not @state.sessionLoaded
