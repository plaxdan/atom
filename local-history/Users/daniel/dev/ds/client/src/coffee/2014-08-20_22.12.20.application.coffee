...
StartCenter = require './ui/startcenter'
...
class DataSpliceApplication extends Backbone.Router
  ...
  doHome: ->
    $.when(@validate()).done =>
      @reactUI.setProps
        component: StartCenter
          factory: @viewFactory
          performAction: _.bind @displayNavigationAction, @

      # First the DS_HOME_BANNER
      {eventFactory} = @appController
      context = eventFactory.context()
      $.when(ExpressionEvaluator.evaluateAttribute 'DS_HOME_BANNER', null,
        { context }
      ).done (startCenterBanner) =>
        # TODO: pass through UI and down to startCenter
        @reactUI.setProps
          component: StartCenter
            factory: @viewFactory
            performAction: _.bind @displayNavigationAction, @
            startCenterBanner: startCenterBanner

      # Second, the actions for each startcenter category
      {configurationManager} = @appController
      list = []
      async.eachSeries categories, (cat, done) =>
          $.when(config.getHomescreenActionsForCategory cat)
            .done( (actions) =>
              return unless actions.length

              list.push { name: cat, actions }
            )
            .always -> done()
        ...
        , =>
          @reactUI.setProps
            component: StartCenter
              factory: @viewFactory
              performAction: _.bind @displayNavigationAction, @
              # TODO: ensure this start center banner is actually set
              startCenterBanner: startCenterBanner
              categories: list
    ...
    $.when(@validate()).done =>
      try
        ...
        @reactUI.setProps
          component: Content
            factory: @viewFactory
            contentView: @viewFactory.create ApplicationSettingsView, { path }
      catch error
  ...
  doDisplayView: (path, query) ->
    $.when(@validate()).done =>
      ...
      @reactUI.setProps
        component: Content
          factory: @viewFactory
          contentView: @viewFactory.create CompositeDataView,
            { state }
