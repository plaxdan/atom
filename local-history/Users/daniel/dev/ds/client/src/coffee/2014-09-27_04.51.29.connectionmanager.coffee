...
class ConnectionManager
    ...
    unless options?.silent or @_unloaded
      ...
      unless session.isEmpty()
        if (session.get 'mode') is 'offline'
          ...
          @dataSpliceWebService.disconnectSession()
  ...
  monitorConnection: ->
    ...
    #
    # @_currentPush = @dataSpliceWebService.getPush()
    #   .then (result) ->
    #     waitForPushResponse result
    #     checkHandlingOfPushResponse()
    #   .catch (error) ->
    ...
    tryHandlePushResponse = (response) =>
      wait = @_handlePushResponse response if response

    tryReconnect = (error) =>
      unless error.response?.status is 200
        # if xhr.responseText
        #   message = try
    ...
    @dataSpliceWebService.getPush timeout: 30000
      .then (result) =>
        @_handlePush result
        checkHandlingOfPushResponse()
      .catch tryReconnect
    ...
    #   timeout: 30000
    #   dataType: 'json'
    #   success: tryHandlePushResponse
    #   error: tryReconnect
    #   complete: checkHandlingOfPushResponse
  ...
  _handlePush: (result) ->
    { payload } = result
    console.debug "Push Response: ", payload
  ...
  _startSync: ->
    ...
    @_generateOfflineDigest()
      .then @dataSpliceWebService.sync
