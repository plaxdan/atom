...
class ExpressionEvaluator
  ...
  @_memoizedParse = memoize ((statement) -> DataSpliceParser.parse statement), primitive: true, max: 100000
  # @_memoizedParse = _.memoize (statement) -> DataSpliceParser.parse statement

  @parse = (statement) ->
    ast = @_memoizedParse statement
    ...
    new Promise (resolve, reject) ->
      async.each _.keys(attributes),
        (name, done) ->
          ...
          if ExpressionEvaluator.isExpression value
            Promise.resolve ExpressionEvaluator.evaluate value, options
              .then (evaluated) ->
  ...
  @replaceReferences = (statement, options) ->
    ...
    new Promise (resolve, reject) ->
      Promise.resolve ExpressionEvaluator.evaluateAttributes matches, options
        .then (values) ->
  ...
  _processValue: (options, removeAttributes) ->
    ...
    if ExpressionEvaluator.isExpression value
      try
        ...
        Promise.resolve ExpressionEvaluator.evaluate value, _.clone options
          .then (result) =>
