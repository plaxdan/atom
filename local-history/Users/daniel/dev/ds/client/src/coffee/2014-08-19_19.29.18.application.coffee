...
class DataSpliceApplication extends Backbone.Router
  ...
  aboutHandler: =>
    console.debug "ABOUT HANDLER"
    @pubSub.trigger 'displayModal',
      title: 'About DataSplice'
      body: (ok, cb) =>
        ok.append (@viewFactory.create AppAboutView).render()
        cb()
        null
  ...
  displayNotificationDetailsHandler: (notification) ->
    console.debug 'Application: displayNotificationDetailsHandler'
    # @viewFactory.pubSub.trigger 'displayError', notification if notification
