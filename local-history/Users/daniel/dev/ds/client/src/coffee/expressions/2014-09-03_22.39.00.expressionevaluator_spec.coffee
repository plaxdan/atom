...
describe 'Expression Evaluator', ->
  ...
  describe 'Memoization of DataSpliceParser', ->
    ...
    spy = chai.spy DataSpliceParser.parse

    beforeEach ->
      DataSpliceParser.parse = spy
      DataSpliceParser.parse.should.be.spy

    afterEach ->
      DataSpliceParser.parse = originalParse
      DataSpliceParser.parse.should.not.be.spy

    it 'should result in one call DataSpliceParser.parse when given the same arguments multiple times', ->
      expression = 'true or ( 1 + 2 )'
      ExpressionEvaluator.parse expression for count in [1..10]
      spy.should.have.been.called.once

    it 'should result in once call to DataSpliceParser.parse for each unique statement', ->
    ...
    it 'should always return the same value for the given statement', ->
      returnVals = []
      expressionA = 'true or ( 1 + 2 )'
      expressionB = 'false or ( 3 + 2 )'
      for count in [1..10]
        returnVals.push ExpressionEvaluator.parse expressionA
        returnVals.push ExpressionEvaluator.parse expressionB

      (_.uniq returnVals).length.should.equal 2

    it 'should be at least an order of magnitude faster than without memoization', ->
      ...
      rawTime = time -> DataSpliceParser.parse expression for [0..numRepetitions]
      memoizedTime = time -> ExpressionEvaluator.parse expression for [0..numRepetitions]

      console.log "RAW TIME: [#{rawTime}], MEMOIZED TIME: [#{memoizedTime}]"
      ratio = rawTime / memoizedTime
      console.log "RATIO: [#{ratio}]"
      ration.should.be.at.least 10
