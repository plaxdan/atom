...
class DataController
  ...
  initializeViewStorage: (vc) ->
    new Promise (resolve, reject) =>
      console.log 'DataController.initializeViewStorage' if TRACE

      ensureSchemaForView = (view, done) =>
        @storage.ensureSchema view
          .then -> done()

      doneAllSchema = -> resolve true

      # process the existing views and ensure the internal schema is set up
      async.each vc.models, ensureSchemaForView, doneAllSchema
  ...
  _processServerFetchQueue: ->
    ...
    Backbone.sync 'read', query.dataSet,
      ...
      error: (error) =>
        # Don't throw an error if we garbase collected. This means we've just
        # abandoned the whole thing (such as the user logged out)
        unless @_gc
