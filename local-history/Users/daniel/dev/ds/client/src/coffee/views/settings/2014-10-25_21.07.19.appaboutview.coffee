...
class AppAboutView extends BaseView
  ...
  initialize: (options) ->
    ...
    clientVersion = options.dsApp.buildVersion
    buildTime = moment options.dsApp.buildTimestamp
...
class TextSizeSelectionView extends BaseView
  ...
  setTextSize: (ev) ->
    ...
    if key
<<<<<<< HEAD
      @options.flux.intents.system.setApplicationTextSize key, persist: true
=======
      @options.flux.actions.env.setApplicationTextSize key, persist: true
>>>>>>> dev
        .then => @render()
  ...
  render: ->
    ...
    current = (@options.settingsService.getUserSetting 'textSize') or 'normal'
