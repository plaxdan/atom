...
class DataFormView extends BaseFormView
  ...
  focusChanged: ->
    console.debug 'DataFormView.focusChanged'
    field = @focusedField()
    @state.set 'selectedField', field?.input.binding.key

  contentEditableChanged: (ev) ->
    console.debug 'DataFormView.contentEditableChanged' if TRACE
    target = $ ev.target
    test = @lookupField target
    test.toModel() if test

  selectedFieldChanged: ->
    console.debug 'DataFormView.selectedFieldChanged'
