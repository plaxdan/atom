...
class DataSpliceController
  ...
  startSession: (options) ->
    ...
    Promise.resolve(loadServer)
      .then (eitherErrorOrServerAttributes) =>
        sessionChecked = @checkSession options unless options?.serverOnly
      .catch (error) =>
    ...
    @session.fetch
      ...
      error: (model, error) =>
        ...
        if (error.status isnt 400) and @session.loadLocal()
          ...
          loadSession.resolve _.extend options, local: true
        else
          loadSession.reject error
