describe 'Promise Handling', ->
  ...
  it 'should call finally even when an error is thrown', (done) ->
    ...
    .finally (successOrError) ->
      ...
      successOrError.message.should.equal 'fail'
      done()

  it 'should call finally after a call to catch', (done) ->
    success = chai.spy -> alert 'success'
    caught = chai.spy -> alert 'caught'
    p = new Promise (resolve, reject) -> throw new Error 'fail'
    .then (result) ->
      success()
    .catch (err) ->
      caught()
    .finally (successOrFail) ->
      # success.should.have.been.called.once
      caught.should.not.have.been.called.once

    Promise.resolve(p).then -> done()
