...
class ConnectionManager
  ...
  constructor: (@fluxActions, @webService, @legacyFactory) ->
  ...
  ensureConnected: (options) ->
    ...
    tryConnect = =>
      ...
      session.save null,
        success: =>
          # TODO: resume the session through the flux actions
          @fluxActions.session.resume()
  ...
  confirmLogOut: ->
    ...
    $.when(promise).done =>
      ...
      @fluxActions.session.logout force: true
    ...
    if parameters?.credentials
      ...
      if tokens.length is 2
        ...
        ($.when @ensureConnected
          ...
          # TODO: resume the session through the actions layer
          @legacyFactory.pubSub.trigger 'restartSession', authenticated: wasEmpty
