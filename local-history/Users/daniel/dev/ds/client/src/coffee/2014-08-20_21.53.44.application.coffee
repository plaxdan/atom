...
class DataSpliceApplication extends Backbone.Router
  ...
  doHome: ->
    $.when(@validate()).done =>
      ...
      # TODO: we want to kick off the async startcenter population process
      #   and keep updating the props of UI (and in turn the startcenter)
      ...
      $.when(ExpressionEvaluator.evaluateAttribute 'DS_HOME_BANNER', null,
        ...
        @setProps { startCenterBanner }

      # Second, the actions for each start center category
