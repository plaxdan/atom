describe 'Basic Environment Tests', ->
  ...
  it 'should ensure Element.classList works', ->
    console.warn document.documentElement.classList
    expect(document.documentElement.classList).to.exist

  describe 'DEBUG flag', ->
    it 'should set window.React if truthy', ->
      if DEBUG
        console.log 'DEBUG'
        (window.React?).should.be.true

    it 'should NOT set window.React if falsy', ->
      unless DEBUG
        console.log 'NOT DEBUG'
        (window.React?).should.be.false
