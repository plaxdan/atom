  ...
  for vendor in vendors
    ...
    if vendor.bundle?
      bundledVendors.push vendor.bundle.map (asset) ->
        ...
        $.util.log "  Bundling #{bundleFile}"
        bundleFile

    if vendor.include?
      include.dest or= "#{webBuildPath}/vendors/#{vendor.name}"
      for asset in vendor.indlude.assets
        if _.isString asset
          asset = src: "#{vendor.base}/#{asset}", dest: include.dest
        else if _.isObject asset
          asset.src = "#{vendor.base}/#{asset.src}"
          asset.dest or= include.dest
