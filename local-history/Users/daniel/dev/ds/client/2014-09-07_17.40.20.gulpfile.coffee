...
gulp.task 'build:src', ['build:vendor2'], (cb) ->
...
gulp.task 'build:tests', [ 'build:src', 'build:vendor2' ], ->
...
gulp.task 'build:vendor2', ->
  bundledVendors = []
  bundledVendorsRelativeFilePath =
    "#{path.relative '.', vendorBundlePath}/#{vendorBundleFile}"
  ...
  for vendor in vendors
    ...
    if vendor.bundle?
      ...
      bundleFiles = vendor.bundle.map (asset) ->
        bundleFile = "#{vendor.base}/#{asset}"
        $.util.log "    #{bundleFile}"
        bundleFile
      console.debug "BUNDLE FILES: #{bundleFiles}"
      bundledVendors.push bundleFiles
    ...
    if vendor.include?
      ...
      for asset in vendor.include.assets
        ...
        asset.dest or= "#{webBuildPath}/vendors/#{vendor.name}"
  ...
  if bundledVendors.length > 0
    console.log bundledVendors
    gulp.src bundledVendors
