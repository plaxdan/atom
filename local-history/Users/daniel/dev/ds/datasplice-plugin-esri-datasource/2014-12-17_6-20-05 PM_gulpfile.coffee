...
paths =
  ...
  zip:          "datasplice-gis-plugins.zip"
...
gulp.task 'clean', (cb) ->
  del [paths.testResults, paths.distBase, paths.release], cb

gulp.task 'build', ['clean'], (cb) ->
  execute "call msbuild #{paths.solution}
    /p:Configuration=Release;TargetFrameworkVersion=v3.5
    /target:Clean,Build", cb

gulp.task 'test', ['build'], (cb) ->
  execute "call mstest /nologo /
    /testcontainer:#{paths.tests} /
    /resultsfile:#{paths.testResults} /
    /runconfig:testrunconfig.testrunconfig", cb
...
gulp.task 'ci', ['dist'], ->
  gulp.src ["#{paths.distBase}/**/*", "#{paths.leaflet}/**/*"]
    .pipe $.zip paths.zip
