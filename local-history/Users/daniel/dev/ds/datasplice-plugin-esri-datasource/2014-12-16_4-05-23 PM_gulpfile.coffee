...
gulp.task 'build', ['clean'], (cb) ->
  execute "call msbuild #{paths.solution} /p:Configuration=Release;TargetFrameworkVersion=v3.5 /target:Clean,Build /runconfig:testrunconfig.testrunconfig", cb

gulp.task 'test', ['build'], (cb) ->
  execute "call mstest /nologo /testcontainer:#{paths.tests} /resultsfile:#{paths.testResults}", cb
